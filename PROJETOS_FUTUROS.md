...
## 🗺️ Plano de Implantação Gradual do ERP & Transições

Para garantir o sucesso e a aderência da equipe ao sistema, o ERP está sendo implantado de forma gradual, permitindo que a operação comercial e financeira ganhe tração antes da introdução de controles rígidos de chão de fábrica.

### 📍 Fase 1: Foco Comercial, Financeiro e Operação Simplificada *(Em Execução)*
* **Objetivo:** Captar pedidos de venda, faturar (SEFAZ), realizar vendas diretas (PDV Express) e processar o contas a receber/pagar e DRE sem fricção.
* **Abordagem de Estoque Transitória (Modo SIMPLIFICADO):**
  * Implementamos um **switch de controle de estoque/CMV** que permite ao ERP rodar em modo `SIMPLIFICADO`.
  * Neste modo, o CMV é apurado com base no **Custo Médio Simplificado** (ou custo padrão cadastrado no produto), sem exigir que cada saída seja amarrada a um lote de produção física ativo (PEPS/FIFO).
  * A baixa de estoque é registrada com `lote_origem_id = NULL` nas saídas físicas, evitando avisos recorrentes de estoque negativo e CMV estimado na interface de faturamento que geravam ruído operacional na Fase 1.

### 📍 Fase 2: Ativação Completa de Estoque, Produção e Compras *(Fase Futura)*
* **Objetivo:** Habilitar a rastreabilidade completa de lotes de produção (FIFO real), controle rígido de matérias-primas e insumos.
* **Gatilho de Transição (Modo LOTE):**
  * Para virar a chave para o modo `LOTE`, a fábrica de alho **exigirá obrigatoriamente um inventário físico de abertura**.
  * Esse inventário servirá para cadastrar e alinhar no banco de dados todos os saldos e datas de validade reais dos lotes presentes no galpão, garantindo que o algoritmo FIFO apure o CMV real de forma precisa desde o primeiro dia.
  * O switch de configuração na aba *Minha Empresa* apresenta um alerta estético destacando esse requisito antes de qualquer alteração acidental.
